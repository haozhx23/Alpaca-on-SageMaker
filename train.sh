...
DISTRIBUTED_ARGS="--nproc_per_node $SM_NUM_GPUS --master_port 12345"

# stanford_alpaca/ -> /opt/ml/code/stanford_alpaca/

# working dir = /opt/ml/code/

torchrun ${DISTRIBUTED_ARGS} stanford_alpaca/train.py \
    --model_name_or_path "/tmp/llama_pretrain/" \
    --data_path /opt/ml/code/stanford_alpaca/alpaca_data.json \
    --bf16 True \
